...
A_sweep_O = zeros(size(sig_min));
A_sweep_S = zeros(size(sig_min));
...
for i = 2:size(X,2)-2
    for j = 2:size(Y,2)-2
        ...
        q_temp = fcn_inv_O(delta,X_temp,Y_temp);
        J = fcn_JO(q_temp,delta);
        P_temp = [X_temp;Y_temp];
        
        A_sweep_O(i,j) = sweepArea_O(L1,L2,r_temp);
        A_sweep_S(i,j) = sweepArea_S(L1,L2,r_temp);
